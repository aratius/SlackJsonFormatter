...
export default class SlackView extends React.Component<Props> {
   ...
   * チャンネル選択
   */
  private onChooseChannel = (e: any, name: string): void => {
    // イベント伝播禁止
  ...
  handleChooseFile = async (_messages: any) => {
    ...
    if(this.state.messageData.length > 0) messages = messages.concat(this.state.messageData)
  ...
  render() {
    ...
    let messages: Message[] = this.state.messageData
    // 日付ソート
    ...
    // チャンネル絞り込み
    const channelMessages: Message[] = messages.filter((data) => data.channel == this.state.targetChannel)
